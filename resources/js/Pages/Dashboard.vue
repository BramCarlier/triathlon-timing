...
<template>
  ...
  <AppLayout title="Race dashboard">
    <div class="mb-5 flex justify-end"><Link v-if="can('races.create')" href="/races/create" class="btn-primary">Create race</Link></div>
    <div v-if="races.length" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <article v-for="race in races" :key="race.id" class="panel-pad flex flex-col gap-4">
        ...
        <div class="mt-auto grid grid-cols-2 gap-2"><Link v-if="can('timings.record')" :href="`/races/${race.id}/station`" class="btn-primary">Timing station</Link><Link v-if="can('races.control')" :href="`/races/${race.id}/control`" class="btn-secondary">Race control</Link><Link v-if="can('participants.manage')" :href="`/races/${race.id}/participants`" class="btn-secondary">Participants</Link><Link :href="`/races/${race.id}/results`" class="btn-secondary">Results</Link></div>
      </article>
    </div>
