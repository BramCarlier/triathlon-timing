...
<Head :title="`${race.name} timing station`"/><AppLayout :title="`${race.name} · Timing station`">
  <div v-if="feedback" class="fixed inset-x-3 top-20 z-50 mx-auto max-w-xl rounded-2xl border p-4 text-center text-lg font-bold shadow-2xl" :class="feedback.type==='ok'?'border-emerald-400 bg-emerald-700 text-white':feedback.type==='offline'?'border-amber-300 bg-amber-500 text-slate-950':'border-red-400 bg-red-700 text-white'">{{ feedback.message }}</div>
  ...
  <template v-else>
    ...
    <p v-if="race.finished_at" class="mb-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-success">Race finished. The shared clock is stopped. Open Race control to correct timings.</p>
