...
<Head :title="`${race.name} timing station`"/><AppLayout :title="`${race.name} · Timing station`">
  ...
  <template v-else>
    ...
    <p v-if="race.finished_at" class="mb-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-success">Race finished. The shared clock is stopped. <Link :href="`/races/${race.id}/results`" class="font-bold underline">View results</Link> · <Link :href="`/races/${race.id}/control`" class="underline">Correct a timing</Link></p>
    <div class="mb-4 grid gap-3 lg:grid-cols-[1fr_auto]">
      ...
      <section class="panel-pad flex min-w-64 flex-col justify-center"><div class="flex items-center justify-between"><span class="text-sm muted">Connection</span><span class="font-semibold" :class="online?'text-success':'text-warning'">{{ online ? 'Online' : 'Offline' }}</span></div><div class="mt-2 flex items-center justify-between"><span class="text-sm muted">Pending sync</span><span class="font-semibold" :class="pending?'text-warning':'text-secondary'">{{ pending }}</span></div><div class="mt-2"><LiveUpdatesStatus :race-id="race.id"/></div><form class="mt-3" @submit.prevent="selectCheckpoint"><label class="label" for="station-checkpoint">Change checkpoint</label><select id="station-checkpoint" v-model="selectForm.checkpoint_id" class="field text-sm" @change="selectCheckpoint"><option v-for="cp in checkpoints" :key="cp.id" :value="cp.id">{{ cp.name }} · {{ checkpointDistanceText(race, cp) }}</option></select></form></section>
    </div>

    <div class="sticky top-[70px] z-20 mb-3 flex items-center justify-between gap-3 rounded-xl border border-outline bg-surface p-3 shadow-lg"><div class="min-w-0"><strong class="block truncate" :data-discipline="checkpoint.discipline">{{ checkpoint.name }}</strong><span class="text-xs muted">{{ pending?`${pending} waiting to upload`:'All timings saved' }}</span></div><span class="badge">{{ race.finished_at?'Finished':race.started_at?'Timing active':'Awaiting start' }}</span></div>
    <div class="grid gap-4 xl:grid-cols-[1fr_360px]">
      <section class="panel overflow-hidden"><div class="border-b border-outline p-4"><label class="label" for="station-search">Bib number or athlete/team name</label><input id="station-search" v-model="query" autofocus class="field min-h-14 text-xl" placeholder="Search name, team or bib" aria-label="Find participant"></div><div class="max-h-[62vh] overflow-auto p-2"><button v-for="participant in filtered" :key="participant.id" class="mb-2 grid min-h-20 w-full grid-cols-[auto_1fr_auto] items-center gap-4 rounded-2xl border p-4 text-left transition" :class="selected(participant)?'border-emerald-500/30 bg-emerald-500/10 opacity-70':'border-outline bg-surface hover:border-cyan-400 hover:bg-raised active:scale-[.995]'" :disabled="saving.has(participant.id) || selected(participant) || !race.started_at || !!race.finished_at || (!!participant.status && participant.status!=='registered')" @click="record(participant)"><span class="rounded-xl bg-canvas px-3 py-2 font-mono text-xl font-black">{{ bibLabel(participant.bib_number) }}</span><span class="min-w-0"><span class="block truncate text-lg font-bold">{{ participant.name }}</span><span v-if="participant.type==='relay'" class="block truncate text-sm muted">{{ participant.members.map(m=>`${m.discipline}: ${m.name}`).join(' · ') }}</span><span v-else class="block text-sm muted">Solo athlete</span></span><span class="text-sm font-bold" :class="selected(participant)?'text-success':'text-accent'">{{ participant.status && participant.status!=='registered' ? participant.status.toUpperCase() : saving.has(participant.id) ? 'SAVING' : selected(participant) ? 'RECORDED' : race.finished_at ? 'FINISHED' : !race.started_at ? 'WAITING' : 'TAP' }}</span></button><div v-if="!filtered.length" class="p-8 text-center muted">No matching participant.</div></div></section>
      <section class="panel overflow-hidden"><div class="flex items-center justify-between border-b border-outline p-4"><h2 class="font-bold">Your recent taps</h2><span class="text-xs muted">Undo mistakes here</span></div><div v-if="recent.length" class="divide-y divide-outline"><div v-for="timing in recent" :key="timing.client_uuid || timing.id" class="p-4"><div class="flex items-center justify-between gap-3"><div><div class="font-mono text-lg font-bold">{{ timing.entry?.display_name ?? bibLabel(timing.entry?.bib_number) }}</div><div class="font-mono text-sm text-accent">{{ formatDuration(timing.elapsed_ms,2) }}</div></div><div class="text-right"><span v-if="timing.queued" class="mb-1 block text-xs font-semibold text-warning">OFFLINE · PENDING</span><button class="rounded-lg px-3 py-2 text-sm font-semibold text-error hover:bg-red-500/10" @click="undo(timing)">Undo</button></div></div></div></div><div v-else class="p-6 text-center muted">No timings recorded by you here yet.</div></section>
    </div>
    ...
    <div class="mt-4 text-center text-xs text-muted">Keep this page open before moving to a low-connectivity checkpoint. Failed submissions are queued locally and sync when the connection returns.</div>
  </template>
