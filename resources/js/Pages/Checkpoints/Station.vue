...
import { formatDuration, jsonRequest, uuid } from '../../lib';
import type { Checkpoint, Race, StationParticipant } from '../../types';

interface Timing { id?:number; client_uuid:string; elapsed_ms:number; recorded_at:string; entry?:{id:number;bib_number:string;members?:unknown[]}; queued?:boolean }
...
const selectForm = useForm({ checkpoint_id: props.checkpoint?.id ?? props.checkpoints[0]?.id ?? null });
const query = ref('');
...
const { elapsedMs, serverNowMs } = useRaceClock(() => props.race.started_at, () => props.serverNow);
...
async function record(participant: StationParticipant, override = false, clientUuid = uuid()) {
  ...
  if (selected(participant)) { showFeedback('error', `#${participant.bib_number} is already recorded here.`); return; }
  ...
  if (missing.length && !override) {
    ...
    if (!confirm(`Earlier required timing missing: ${names}. Record #${participant.bib_number} here anyway?`)) return;
    override = true;
  }
  ...
  const localTiming: Timing = { client_uuid:clientUuid, elapsed_ms:elapsedMs.value, recorded_at:String(payload.observed_at), entry:{id:participant.id,bib_number:participant.bib_number}, queued:!online.value };
  ...
  if (!online.value) {
    ...
    showFeedback('offline', `#${participant.bib_number} saved offline · ${formatDuration(elapsedMs.value, true)}`);
    query.value = '';
    return;
  }
  ...
  try {
    ...
    if (response.ok && data.timing) {
      participant.completed_checkpoint_ids.push(props.checkpoint.id);
      recent.value.unshift({...data.timing, entry:{id:participant.id,bib_number:participant.bib_number}}); recent.value = recent.value.slice(0,10);
      showFeedback('ok', data.message ?? `#${participant.bib_number} recorded.`); query.value=''; return;
    }
    ...
    showFeedback('offline', `Connection lost. #${participant.bib_number} saved on this device.`); query.value='';
  }
}
...
<Head :title="`${race.name} timing station`"/><AppLayout :title="`${race.name} · Timing station`">
  ...
  <template v-else>
    <div class="mb-4 grid gap-3 lg:grid-cols-[1fr_auto]">
      <section class="panel-pad"><div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div class="text-xs font-semibold uppercase tracking-[.2em] text-cyan-300">{{ checkpoint.name }}</div><div class="mt-1 text-sm muted">{{ checkpoint.discipline ?? 'race' }}<span v-if="checkpoint.distance_km"> · {{ checkpoint.distance_km }} km</span></div></div><RaceClock :started-at="race.started_at" :server-now="serverNow"/></div></section>
    ...
    <div class="grid gap-4 xl:grid-cols-[1fr_360px]">
      <section class="panel overflow-hidden"><div class="border-b border-slate-800 p-4"><label class="label">Bib number or athlete/team name</label><input v-model="query" autofocus inputmode="numeric" class="field min-h-14 text-xl" placeholder="Type bib, e.g. 142"></div><div class="max-h-[62vh] overflow-auto p-2"><button v-for="participant in filtered" :key="participant.id" class="mb-2 grid min-h-20 w-full grid-cols-[auto_1fr_auto] items-center gap-4 rounded-2xl border p-4 text-left transition" :class="selected(participant)?'border-emerald-500/30 bg-emerald-500/10 opacity-70':'border-slate-800 bg-slate-900 hover:border-cyan-400 hover:bg-slate-800 active:scale-[.995]'" :disabled="selected(participant)" @click="record(participant)"><span class="rounded-xl bg-slate-950 px-3 py-2 font-mono text-xl font-black">#{{ participant.bib_number }}</span><span class="min-w-0"><span class="block truncate text-lg font-bold">{{ participant.name }}</span><span v-if="participant.type==='relay'" class="block truncate text-sm muted">{{ participant.members.map(m=>`${m.discipline}: ${m.name}`).join(' · ') }}</span><span v-else class="block text-sm muted">Solo athlete</span></span><span class="text-sm font-bold" :class="selected(participant)?'text-emerald-300':'text-cyan-300'">{{ selected(participant) ? 'RECORDED' : 'TAP' }}</span></button><div v-if="!filtered.length" class="p-8 text-center muted">No matching participant.</div></div></section>
      <section class="panel overflow-hidden"><div class="flex items-center justify-between border-b border-slate-800 p-4"><h2 class="font-bold">Your recent taps</h2><span class="text-xs muted">Undo mistakes here</span></div><div v-if="recent.length" class="divide-y divide-slate-800"><div v-for="timing in recent" :key="timing.client_uuid || timing.id" class="p-4"><div class="flex items-center justify-between gap-3"><div><div class="font-mono text-lg font-bold">#{{ timing.entry?.bib_number }}</div><div class="font-mono text-sm text-cyan-200">{{ formatDuration(timing.elapsed_ms,true) }}</div></div><div class="text-right"><span v-if="timing.queued" class="mb-1 block text-xs font-semibold text-amber-300">OFFLINE · PENDING</span><button class="rounded-lg px-3 py-2 text-sm font-semibold text-red-300 hover:bg-red-500/10" @click="undo(timing)">Undo</button></div></div></div></div><div v-else class="p-6 text-center muted">No timings recorded by you here yet.</div></section>
    </div>
