...
<Head :title="`${race.name} timing station`"/><AppLayout :title="`${race.name} · Timing station`">
  ...
  <template v-else>
    ...
    <p v-if="race.finished_at" class="mb-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-success">Race finished. The shared clock is stopped. <Link :href="`/races/${race.id}/results`" class="font-bold underline">View results</Link> <Link v-if="can('races.control')" :href="`/races/${race.id}/control`" class="underline">Correct a timing</Link></p>
