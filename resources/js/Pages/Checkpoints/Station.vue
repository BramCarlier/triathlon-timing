  ...
  <template v-else>
    ...
    <section class="panel-pad mb-4" aria-label="Device timing recovery">
      ...
      <p class="mb-3 font-semibold" role="status">{{ pending ? `${pending} timings waiting to upload` : 'All timings saved' }}</p><div class="flex flex-wrap items-center gap-3">
        <button class="btn-secondary" :disabled="flushing || !online || !pending" @click="syncPending"><i class="fa-solid fa-cloud-arrow-up" aria-hidden="true"></i>{{ flushing ? 'Uploading…' : 'Upload now' }}</button>
        <button v-if="pending || (account.role==='admin' && legacy.length)" class="btn-secondary" @click="exportPending"><i class="fa-solid fa-file-export" aria-hidden="true"></i>Export pending backup</button>
