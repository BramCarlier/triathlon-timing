...
const fullscreen=async()=>{try { if(document.fullscreenElement)await document.exitFullscreen();else await document.documentElement.requestFullscreen(); }catch{screenError.value=tr('Fullscreen is not available in this browser. Display mode still works.');}};
...
<template>
<Head :title="`${race.name} · ${$t('Results')}`"/><AppLayout :title="`${race.name} · ${$t('Results')}`" :public-view="publicMode" :display-mode="display">
  <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><span class="badge" :data-status="race.status">{{ race.finished_at?$t('Race closed'):race.started_at?$t('Live results'):$t('Awaiting start') }}</span><p class="mt-2 text-sm muted">{{ formatDate(race.event_date) }} {{ $t("· Updates automatically.") }} {{ race.finished_at?$t('Corrections may still change results.'):'' }}</p></div><div v-if="display" class="flex flex-wrap gap-2"><button class="btn-primary" @click="toggleDisplay"><i class="fa-solid fa-compress" aria-hidden="true"></i>{{ $t("Exit display mode") }}</button><button class="btn-icon" :aria-label="$t('Toggle fullscreen')" :title="$t('Toggle fullscreen')" @click="fullscreen"><i class="fa-solid fa-expand" aria-hidden="true"></i></button></div></div>
  ...
  <template v-for="row in visibleRows" :key="row.id"><tr class="border-t border-outline"><td class="sticky left-0 z-10 bg-surface p-3 font-bold">{{ row.place??'—' }}</td><td class="sm:sticky left-20 z-10 max-w-56 bg-surface p-3"><div class="font-semibold break-words">{{ row.name }}</div><div class="mt-1 text-xs muted">{{ bibLabel(row.bib_number) }} · {{ row.category }} · {{ $t(row.result_status) }}</div></td><td class="whitespace-nowrap p-3 font-mono font-bold tabular-nums" :class="row.finished?'text-success':'text-muted'">{{ formatDuration(row.total_ms,precision) }}</td><td class="whitespace-nowrap p-3 font-mono tabular-nums">{{ row.gap_ms===0?$t('Leader'):row.gap_ms==null?'—':'+'+formatDuration(row.gap_ms,precision) }}</td><td v-if="!display" class="p-3"><button class="btn-secondary whitespace-nowrap" :aria-expanded="expanded.includes(row.id)" :aria-controls="`splits-${row.id}`" @click="toggleRow(row.id)"><i :class="expanded.includes(row.id)?'fa-solid fa-eye-slash':'fa-solid fa-eye'" aria-hidden="true"></i>{{ expanded.includes(row.id)?$t('Hide splits'):$t('View splits') }}</button></td></tr>
  <tr v-if="!display&&expanded.includes(row.id)"><td colspan="5" class="bg-canvas p-4"><ResultSplits :id="`splits-${row.id}`" :race="race" :checkpoints="checkpoints" :splits="row.splits" :precision="precision"/><p v-if="row.type==='relay'" class="mt-3 text-sm muted">{{ row.members.map(member=>`${$t(member.discipline)}: ${member.name}`).join(' · ') }}</p></td></tr></template>
  <tr v-if="!results.length"><td :colspan="display?4:5" class="p-8 text-center muted">{{ filters.type||filters.category||filters.status?$t('No participants match these filters.'):$t('No participants yet. Results will appear here when participants are added.') }}</td></tr>
  </tbody></table></div>
  <div v-if="display" class="mt-4 flex flex-wrap items-center justify-center gap-3"><button class="btn-secondary" @click="pageNumber=(pageNumber+pages-1)%pages"><i class="fa-solid fa-chevron-left" aria-hidden="true"></i>{{ $t("Previous page") }}</button><span>{{ $t('Page :page of :pages', {page:pageNumber%pages+1,pages}) }}</span><button class="btn-secondary" @click="pageNumber=(pageNumber+1)%pages">{{ $t("Next page") }}<i class="fa-solid fa-chevron-right" aria-hidden="true"></i></button></div>
</AppLayout></template>
