...
<Head :title="`${race.name} results`"/><AppLayout :title="`${race.name} · Results`" :public-view="publicMode" :display-mode="display">
  ...
  <section v-if="!display" class="space-y-3 sm:hidden" aria-label="Race results"><article v-for="row in visibleRows" :key="row.id" class="panel-pad"><div class="flex items-start gap-3"><div class="shrink-0 text-center"><span class="block text-xs muted">{{ $t("Place") }}</span><strong class="text-2xl">{{ row.place??'—' }}</strong></div><div class="min-w-0"><h2 class="font-bold">{{ row.name }}</h2><p class="mt-1 text-sm muted">{{ bibLabel(row.bib_number) }} · {{ row.category }} · {{ row.result_status }}</p></div></div><dl class="mt-4 grid grid-cols-1 gap-3 min-[360px]:grid-cols-2"><div><dt class="text-xs muted">{{ $t("Total time") }}</dt><dd class="font-mono text-lg font-bold tabular-nums" :class="row.finished?'text-success':'text-muted'">{{ formatDuration(row.total_ms,precision) }}</dd></div><div><dt class="text-xs muted">{{ $t("Gap to leader") }}</dt><dd class="font-mono tabular-nums">{{ row.gap_ms===0?$t('Leader'):row.gap_ms==null?'—':'+'+formatDuration(row.gap_ms,precision) }}</dd></div></dl><button class="btn-secondary mt-4 w-full" :aria-expanded="expanded.includes(row.id)" :aria-controls="`mobile-splits-${row.id}`" @click="toggleRow(row.id)"><i :class="expanded.includes(row.id)?'fa-solid fa-eye-slash':'fa-solid fa-eye'" aria-hidden="true"></i>{{ expanded.includes(row.id)?'Hide splits':'View splits' }}</button><div v-if="expanded.includes(row.id)" :id="`mobile-splits-${row.id}`" class="mt-3"><ResultSplits :race="race" :checkpoints="checkpoints" :splits="row.splits" :precision="precision"/><p v-if="row.type==='relay'" class="mt-3 text-sm muted">{{ row.members.map(member=>`${member.discipline}: ${member.name}`).join(' · ') }}</p></div></article><p v-if="!results.length" class="panel-pad muted">{{ $t("No participants match these results yet.") }}</p></section>
