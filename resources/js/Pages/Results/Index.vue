...
<Head :title="`${race.name} results`"/><AppLayout :title="`${race.name} · Results`" :public-view="publicMode" :display-mode="display">
  <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><span class="badge" :data-status="race.status">{{ race.finished_at?$t('Race closed'):race.started_at?'Live results':'Awaiting start' }}</span><p class="mt-2 text-sm muted">{{ formatDate(race.event_date) }} · Updates automatically. {{ race.finished_at?$t('Corrections may still change results.'):'' }}</p></div><div v-if="display" class="flex flex-wrap gap-2"><button class="btn-primary" @click="toggleDisplay"><i class="fa-solid fa-compress" aria-hidden="true"></i>{{ $t("Exit display mode") }}</button><button class="btn-icon" :aria-label="$t('Toggle fullscreen')" :title="$t('Toggle fullscreen')" @click="fullscreen"><i class="fa-solid fa-expand" aria-hidden="true"></i></button></div></div>
  <p v-if="screenError" role="status" class="mb-4 text-warning">{{ screenError }}</p>
  <section v-show="!display" class="panel-pad mb-4 grid gap-3 sm:grid-cols-3" :aria-label="$t('Result filters')"><label class="label">{{ $t("Participant type") }}<select v-model="filters.type" class="field"><option value="">{{ $t("All types") }}</option><option value="solo">{{ $t("Solo") }}</option><option value="relay">{{ $t("Relay") }}</option></select></label><label class="label">{{ $t("Category") }}<select v-model="filters.category" class="field"><option value="">{{ $t("All categories") }}</option><option v-for="category in categories" :key="category">{{ category }}</option></select></label><label class="label">{{ $t("Result status") }}<select v-model="filters.status" class="field"><option value="">{{ $t("All statuses") }}</option><option>{{ $t("FINISHED") }}</option><option>{{ $t("IN PROGRESS") }}</option><option>DNS</option><option>DNF</option><option>DSQ</option></select></label><p class="muted text-xs sm:col-span-3">{{ $t("Places and gaps apply to the selected group. Equal total milliseconds share a place (1, 1, 3). DNS, DNF and DSQ keep their timings but receive no place.") }}</p></section>
  <div v-if="!display" class="mb-4 flex flex-wrap gap-2">
    <details class="rounded-xl border border-outline bg-surface p-2">
      ...
      <div class="mt-3 w-full max-w-sm space-y-3 p-2">
        <label class="label">{{ $t("Timing precision") }}<select v-model="precision" class="field mt-1"><option :value="2">{{ $t("Hundredths (0.01 s)") }}</option><option :value="3">{{ $t("Milliseconds (0.001 s)") }}</option></select></label>
        <p class="text-xs muted">{{ $t("Places always use full millisecond precision.") }}</p>
  ...
  <div v-else class="mb-4 flex flex-wrap items-center gap-3"><label class="flex items-center gap-2"><input v-model="rotate" type="checkbox">{{ $t("Rotate pages every 15 seconds") }}</label><span class="text-sm muted">Timing precision: {{ precision===3?'milliseconds':'hundredths' }}</span></div>
  <p v-if="display" class="mb-3 text-lg font-semibold">{{ filters.category||$t('All categories') }} · {{ filters.type||$t('All participant types') }} · {{ filters.status||$t('All statuses') }}</p>
  <section v-if="!display" class="space-y-3 sm:hidden" :aria-label="$t('Race results')"><article v-for="row in visibleRows" :key="row.id" class="panel-pad"><div class="flex items-start gap-3"><div class="shrink-0 text-center"><span class="block text-xs muted">{{ $t("Place") }}</span><strong class="text-2xl">{{ row.place??'—' }}</strong></div><div class="min-w-0"><h2 class="font-bold">{{ row.name }}</h2><p class="mt-1 text-sm muted">{{ bibLabel(row.bib_number) }} · {{ row.category }} · {{ row.result_status }}</p></div></div><dl class="mt-4 grid grid-cols-1 gap-3 min-[360px]:grid-cols-2"><div><dt class="text-xs muted">{{ $t("Total time") }}</dt><dd class="font-mono text-lg font-bold tabular-nums" :class="row.finished?'text-success':'text-muted'">{{ formatDuration(row.total_ms,precision) }}</dd></div><div><dt class="text-xs muted">{{ $t("Gap to leader") }}</dt><dd class="font-mono tabular-nums">{{ row.gap_ms===0?$t('Leader'):row.gap_ms==null?'—':'+'+formatDuration(row.gap_ms,precision) }}</dd></div></dl><button class="btn-secondary mt-4 w-full" :aria-expanded="expanded.includes(row.id)" :aria-controls="`mobile-splits-${row.id}`" @click="toggleRow(row.id)"><i :class="expanded.includes(row.id)?'fa-solid fa-eye-slash':'fa-solid fa-eye'" aria-hidden="true"></i>{{ expanded.includes(row.id)?'Hide splits':'View splits' }}</button><div v-if="expanded.includes(row.id)" :id="`mobile-splits-${row.id}`" class="mt-3"><ResultSplits :race="race" :checkpoints="checkpoints" :splits="row.splits" :precision="precision"/><p v-if="row.type==='relay'" class="mt-3 text-sm muted">{{ row.members.map(member=>`${member.discipline}: ${member.name}`).join(' · ') }}</p></div></article><p v-if="!results.length" class="panel-pad muted">{{ $t("No participants match these results yet.") }}</p></section>
  <p v-if="display" class="mb-2 text-sm muted sm:hidden">{{ $t("Swipe the results table sideways to see all columns.") }}</p>
  <div class="scroll-region panel overflow-x-auto" :class="display?'':'hidden sm:block'" tabindex="0" role="region" :aria-label="$t('Results table')"><table class="w-full min-w-[620px] text-left" :class="display?'text-xl':'text-sm'"><caption class="sr-only">{{ $t("Race results ordered by finish time, with expandable checkpoint timings") }}</caption><thead class="bg-surface text-xs uppercase tracking-wide text-muted"><tr><th class="sticky left-0 z-20 w-20 min-w-20 bg-surface p-3 whitespace-nowrap">{{ $t("Place") }}</th><th class="sm:sticky left-20 z-20 min-w-40 bg-surface p-3">{{ $t("Participant") }}</th><th class="p-3">{{ $t("Total time") }}</th><th class="p-3">{{ $t("Gap to leader") }}</th><th v-if="!display" class="p-3">{{ $t("Checkpoints") }}</th></tr></thead><tbody>
  <template v-for="row in visibleRows" :key="row.id"><tr class="border-t border-outline"><td class="sticky left-0 z-10 bg-surface p-3 font-bold">{{ row.place??'—' }}</td><td class="sm:sticky left-20 z-10 max-w-56 bg-surface p-3"><div class="font-semibold break-words">{{ row.name }}</div><div class="mt-1 text-xs muted">{{ bibLabel(row.bib_number) }} · {{ row.category }} · {{ row.result_status }}</div></td><td class="whitespace-nowrap p-3 font-mono font-bold tabular-nums" :class="row.finished?'text-success':'text-muted'">{{ formatDuration(row.total_ms,precision) }}</td><td class="whitespace-nowrap p-3 font-mono tabular-nums">{{ row.gap_ms===0?$t('Leader'):row.gap_ms==null?'—':'+'+formatDuration(row.gap_ms,precision) }}</td><td v-if="!display" class="p-3"><button class="btn-secondary whitespace-nowrap" :aria-expanded="expanded.includes(row.id)" :aria-controls="`splits-${row.id}`" @click="toggleRow(row.id)"><i :class="expanded.includes(row.id)?'fa-solid fa-eye-slash':'fa-solid fa-eye'" aria-hidden="true"></i>{{ expanded.includes(row.id)?'Hide splits':'View splits' }}</button></td></tr>
