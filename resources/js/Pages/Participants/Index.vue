...
<template><Head :title="`${race.name} participants`"/><AppLayout :title="`${race.name} · Participants`">
  ...
  <div class="grid gap-5 xl:grid-cols-[1fr_420px]">
    <section class="panel overflow-hidden"><div class="border-b border-outline p-4"><div class="flex items-center gap-3"><input aria-label="Search participants" v-model="searchValue" class="field" placeholder="Search bib, athlete or team…"><span class="whitespace-nowrap muted text-sm">{{ entries.total }} entries</span></div></div><div class="divide-y divide-outline"><div v-for="entry in entries.data" :key="entry.id" class="flex items-start gap-4 p-4"><div class="rounded-xl bg-raised px-3 py-2 font-mono text-lg font-bold">{{ bibLabel(entry.bib_number) }}</div><div class="min-w-0 flex-1"><div class="font-semibold">{{ displayName(entry) }}</div><div class="muted text-sm">{{ entry.type === 'relay' ? 'Trio relay' : 'Solo' }}<span v-if="entry.category"> · {{ entry.category }}</span></div><div v-if="entry.type === 'relay'" class="mt-1 flex flex-wrap gap-x-3 text-xs text-muted"><span v-for="member in entry.members" :key="member.id">{{ disciplineLabel(member.discipline) }}: {{ member.athlete.full_name }}</span></div></div><Link class="btn-secondary" :href="`/races/${race.id}/participants/${entry.id}/edit`">Edit</Link><button class="rounded-lg px-2 py-1 text-sm text-error hover:bg-red-500/10" @click="remove(entry)">Remove</button></div><div v-if="!entries.data.length" class="p-8 text-center muted">No participants found.</div></div><div class="flex items-center justify-between border-t border-outline p-4"><Link v-if="entries.prev_page_url" :href="entries.prev_page_url" class="btn-secondary">Previous</Link><span v-else></span><span class="muted text-sm">Page {{ entries.current_page }} of {{ entries.last_page }}</span><Link v-if="entries.next_page_url" :href="entries.next_page_url" class="btn-secondary">Next</Link><span v-else></span></div></section>
    <form class="panel-pad h-fit" @submit.prevent="submit"><h2 class="mb-4 text-lg font-bold">Add manually</h2><div class="grid grid-cols-2 gap-3"><div><label for="participant-bib" class="label">Bib number (optional)</label><input id="participant-bib" v-model="form.bib_number" class="field" maxlength="32" placeholder="Leave blank if not assigned"></div><div><label for="participants-index-field-1" class="label">Type</label><select id="participants-index-field-1" v-model="form.type" class="field"><option value="solo">Solo</option><option value="relay">Trio relay</option></select></div><div v-if="form.type==='relay'" class="col-span-2"><label for="participants-index-field-2" class="label">Team name</label><input id="participants-index-field-2" v-model="form.team_name" class="field" required></div><div class="col-span-2"><label for="participants-index-field-3" class="label">Category</label><input id="participants-index-field-3" v-model="form.category" class="field" placeholder="Optional"></div></div><div class="mt-5 space-y-4"><div v-for="(member,index) in form.members" :key="member.discipline" class="rounded-xl border border-outline p-3"><div class="mb-3 font-semibold">{{ form.type==='solo' ? 'Athlete' : disciplineLabel(member.discipline) }}</div><div class="grid grid-cols-2 gap-2"><label class="label ">First name<input v-model="member.first_name" class="field" placeholder="First name" required></label><label class="label ">Last name<input v-model="member.last_name" class="field" placeholder="Last name" required></label><label class="label col-span-2">Email (optional)<input v-model="member.email" class="field col-span-2" type="email" placeholder="Email (optional)"></label><label class="label col-span-2">Club (optional)<input v-model="member.club" class="field col-span-2" placeholder="Club (optional)"></label></div></div></div><p v-if="Object.keys(form.errors).length" class="mt-3 text-sm text-error">{{ Object.values(form.errors)[0] }}</p><button class="btn-primary mt-5 w-full" :disabled="form.processing">Add participant</button></form>
  </div>
