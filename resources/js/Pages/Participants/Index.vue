...
<template><Head :title="`${race.name} participants`"/><AppLayout :title="`${race.name} · Participants`">
  ...
  <div class="grid gap-5 xl:grid-cols-[1fr_420px]">
    <section class="panel overflow-hidden"><div class="border-b border-outline p-4"><div class="flex items-center gap-3"><input aria-label="Search participants" v-model="searchValue" class="field" placeholder="Search bib, athlete or team…"><span class="whitespace-nowrap muted text-sm">{{ entries.total }} entries</span></div></div><div class="divide-y divide-outline"><div v-for="entry in entries.data" :key="entry.id" class="grid grid-cols-[auto_minmax(0,1fr)] items-start gap-3 p-4 sm:grid-cols-[auto_minmax(0,1fr)_auto_auto]"><div class="max-w-24 rounded-xl bg-raised px-3 py-2 font-mono text-lg font-bold">{{ bibLabel(entry.bib_number) }}</div><div class="min-w-0 flex-1"><div class="font-semibold">{{ displayName(entry) }}</div><div class="muted text-sm">{{ entry.type === 'relay' ? 'Trio relay' : 'Solo' }}<span v-if="entry.category"> · {{ entry.category }}</span></div><div v-if="entry.type === 'relay'" class="mt-1 flex flex-wrap gap-x-3 text-xs text-muted"><span v-for="member in entry.members" :key="member.id">{{ disciplineLabel(member.discipline) }}: {{ member.athlete.full_name }}</span></div></div><Link class="btn-secondary" :href="`/races/${race.id}/participants/${entry.id}/edit`">Edit</Link><button class="min-h-11 rounded-lg px-3 py-2 text-sm text-error hover:bg-red-500/10" @click="remove(entry)">Remove</button></div><div v-if="!entries.data.length" class="p-8 text-center muted">No participants found.</div></div><div class="flex flex-wrap items-center justify-between gap-2 border-t border-outline p-4"><Link v-if="entries.prev_page_url" :href="entries.prev_page_url" class="btn-secondary">Previous</Link><span v-else></span><span class="muted text-sm">Page {{ entries.current_page }} of {{ entries.last_page }}</span><Link v-if="entries.next_page_url" :href="entries.next_page_url" class="btn-secondary">Next</Link><span v-else></span></div></section>
