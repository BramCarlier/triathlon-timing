...
interface EntryOption { id:number; bib_number:string|null; name:string }
...
<template><Head :title="`${race.name} control`"/><AppLayout :title="`${race.name} · Race control`">
  <div class="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
    <section class="panel-pad"><div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><div class="mb-2 text-xs font-semibold uppercase tracking-[.2em] text-cyan-300">Shared race clock</div><RaceClock :started-at="race.started_at" :finished-at="race.finished_at" :server-now="serverNow"/><LiveUpdatesStatus :race-id="race.id"/></div><div class="flex gap-2"><button v-if="!race.started_at" class="btn-primary min-w-32" @click="pendingAction='start'">Start race</button><button v-else-if="!race.finished_at" class="btn-danger" @click="pendingAction='finish'">Finish race</button><span v-else class="badge">Finished</span></div></div><div class="mt-6 grid grid-cols-3 gap-3"><div class="rounded-xl bg-slate-950 p-4"><div class="text-2xl font-bold">{{ race.entries_count }}</div><div class="muted text-sm">Entries</div></div><div class="rounded-xl bg-slate-950 p-4"><div class="text-2xl font-bold">{{ completedCount }}</div><div class="muted text-sm">Finished</div></div><div class="rounded-xl bg-slate-950 p-4"><div class="text-2xl font-bold">{{ presence.length }}</div><div class="muted text-sm">Active stations</div></div></div></section>
  ...
  <ConfirmDialog v-if="pendingAction" :title="pendingAction==='start' ? 'Start race?' : 'Finish race?'"
    ...
    <p v-if="Object.keys(clockForm.errors).length" class="mt-3 text-red-300">{{ Object.values(clockForm.errors)[0] }}</p>
  </ConfirmDialog>
</AppLayout></template>
