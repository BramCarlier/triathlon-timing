  ...
  <AppLayout :title="$t('Races')">
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-sm muted">{{ isAdmin?'Athletes, checkpoints, the shared race-day link and the race clock stay together. Official accounts are optional.':$t('Your checkpoint assignment follows your account automatically.') }}</p>
      </div>
    ...
    <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      <article v-for="race in races" :key="race.id" class="panel-pad flex flex-col gap-4">
        ...
        <Link :href="isAdmin?`/races/${race.id}`:`/races/${race.id}/station`" class="btn-primary mt-auto">
          <i :class="isAdmin?'fa-solid fa-arrow-right':'fa-solid fa-stopwatch'" aria-hidden="true"></i>{{ isAdmin?(race.finished_at?'Open finished race':$t('Open race')):$t('Open timing station') }}
        </Link>
      </article>
    </div>
