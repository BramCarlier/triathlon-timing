...
<template>
 <Head :title="$t('Official access')"/><AppLayout :title="$t('Official access')">
  ...
  <div class="grid gap-5 lg:grid-cols-2"><section class="panel-pad"><h2 class="text-lg font-bold">{{ $t("Access presets") }}</h2>
   <article v-for="role in roles" :key="role.id" class="border-b border-outline py-4"><div class="flex flex-wrap items-center justify-between gap-3"><h3 class="font-bold">{{ role.name }} <span v-if="role.is_default" class="badge">{{ $t("Default") }}</span></h3><div class="flex flex-wrap gap-2"><button class="btn-secondary" @click="edit(role)" :aria-label="$t('Edit :name', {name:role.name})"><i class="fa-solid fa-pen-to-square" aria-hidden="true"></i>{{ $t("Edit") }}</button><button v-if="!role.is_default" class="btn-danger" :disabled="role.users_count>0" @click="deletion.clearErrors();removing=role" :aria-label="$t('Delete :name', {name:role.name})"><i class="fa-solid fa-trash" aria-hidden="true"></i>{{ $t("Delete") }}</button></div></div><p class="mt-2 muted">{{ $t(role.description ?? '') }}</p><p class="mt-2 text-sm muted">{{ role.users_count }} {{ $t("assigned users ·") }} {{ role.permissions.length }} {{ $t("permissions") }}</p><p v-if="!role.is_default && role.users_count" class="mt-1 text-xs muted">{{ $t("Reassign these users before deleting this role.") }}</p><ul class="mt-2 list-disc pl-5 text-sm"><li v-for="key in role.permissions" :key="key">{{ permissions[key]?.label??key }}</li></ul></article>
  ...
  <ConfirmDialog v-if="removing" :title="$t('Delete preset?')" :message="$t('Delete :name? You can create a new role later.', {name:removing.name})" :confirm-label="$t('Delete preset')" :busy="deletion.processing" @cancel="removing=null" @confirm="remove"><p v-if="Object.keys(deletion.errors).length" role="alert" class="text-error">{{ Object.values(deletion.errors)[0] }}</p></ConfirmDialog>
 </AppLayout>
</template>
