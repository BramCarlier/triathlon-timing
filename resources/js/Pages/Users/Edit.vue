...
<template>
  ...
  <AppLayout title="Edit person & access">
    <Link href="/users" class="btn-secondary mb-5"><i class="fa-solid fa-arrow-left" aria-hidden="true"></i>Back to People</Link>

    <section v-if="account.force_password_change" class="panel-pad mb-5">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><h2 class="font-bold">Account setup pending</h2><p class="mt-1 text-sm muted">{{ account.invitation_sent_at?'An invitation was sent. You can send a fresh link if needed.':'This user must choose a password before using the app.' }}</p></div>
        <button type="button" class="btn-secondary" :disabled="!mailConfigured || !account.is_active" @click="router.post(`/users/${account.id}/invitation`)"><i class="fa-solid fa-envelope" aria-hidden="true"></i>Resend invitation</button>
      </div>
    </section>
    ...
    <form class="grid gap-5 lg:grid-cols-2" @submit.prevent="save">
      <section class="panel-pad space-y-4">
        ...
        <label class="label">Name<input v-model="form.name" class="field" required maxlength="255"></label>
        <label class="label">Email<input v-model="form.email" type="email" class="field" required maxlength="255"></label>
        <label class="label">Account type<select v-model="form.role" class="field"><option value="admin">Organizer (admin)</option><option value="organizer">Official</option><option value="athlete">Athlete</option></select></label>
        <label class="flex items-center gap-3"><input v-model="form.is_active" type="checkbox"><span><strong class="block text-sm">Account active</strong><span class="text-xs muted">Disabled accounts cannot sign in.</span></span></label>
        <details class="rounded-xl border border-outline p-3">
          <summary class="font-semibold">Password options</summary>
          <label class="label mt-3">New temporary password <span class="font-normal muted">(optional)</span><input v-model="form.password" type="password" class="field" autocomplete="new-password" minlength="12"></label>
          <p class="mt-2 text-xs muted">Leave blank to keep the current password. A new temporary password must be changed at the next sign-in.</p>
        </details>
      </section>
      ...
      <section class="panel-pad space-y-4">
        ...
        <template v-else-if="form.role==='organizer'">
          <p class="text-sm muted">Checkpoint assignments normally grant race access automatically. Only change the options below for exceptional access.</p>
          <details class="rounded-xl border border-outline p-3">
            ...
            <h3 class="mt-4 text-sm font-semibold">Extra race access</h3>
            <p class="mt-1 text-xs muted">Use only when this Official needs race access without a checkpoint assignment.</p>
