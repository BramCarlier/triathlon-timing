...
<template>
  <div class="space-y-3">
    <div v-if="selected && model===selected.id" class="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3">
      <div class="flex items-start justify-between gap-3"><div><span class="text-xs font-bold uppercase tracking-wider text-success">{{ $t("Athlete selected") }}</span><strong class="mt-1 block">{{ selected.first_name }} {{ selected.last_name }}</strong><span class="text-sm muted">{{ selected.email || $t('No email') }}</span></div><button type="button" class="btn-secondary !px-3" @click="clear">{{ $t("Change") }}</button></div>
    </div>
    <template v-else>
      <label for="athlete-search" class="label">{{ $t("Athlete") }}</label>
      <input id="athlete-search" v-model="query" maxlength="100" class="field" :placeholder="$t('Filter by name or email')" autocomplete="off">
      <p class="text-xs muted">{{ $t("Available athlete profiles are shown below and filtered as you type.") }}</p>
      <p v-if="busy" class="text-sm muted"><i class="fa-solid fa-spinner fa-spin mr-1" aria-hidden="true"></i>{{ $t("Loading athletes…") }}</p>
      ...
      <div v-if="!busy && rows.length" class="max-h-64 space-y-1 overflow-auto rounded-xl border border-outline bg-canvas p-2">
        <button v-for="athlete in rows" :key="athlete.id" type="button" class="w-full rounded-xl border border-outline p-3 text-left hover:border-cyan-400 hover:bg-raised" @click="choose(athlete)">
          <strong>{{ athlete.first_name }} {{ athlete.last_name }}</strong><span class="mt-1 block text-xs muted">{{ athlete.email || $t('No email') }}</span>
        </button>
      </div>
      <p v-else-if="!busy&&!error" class="rounded-xl bg-canvas p-3 text-sm muted">{{ $t("No available athlete matches.") }}</p>
      <div v-if="last>1" class="flex items-center justify-between gap-2"><button type="button" class="btn-secondary" :disabled="busy||page<=1" @click="load(page-1)">{{ $t("Previous") }}</button><span class="text-xs muted">{{ page }} / {{ last }}</span><button type="button" class="btn-secondary" :disabled="busy||page>=last" @click="load(page+1)">{{ $t("Next") }}</button></div>
    </template>
  </div>
</template>
