...
export function useWakeLock() {
  ...
  const acquire=async()=>{if(!enabled.value||document.visibilityState!=='visible')return;try{lock=await navigator.wakeLock.request('screen');active.value=true;lock.addEventListener('release',()=>{active.value=false;});error.value='';}catch{active.value=false;error.value='Screen wake lock is unavailable. Check your device power settings.';}};
