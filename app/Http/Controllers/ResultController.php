...
class ResultController extends Controller
{
    ...
    public function publicIndex(Request $request,string $token, ResultsService $results): Response
    {
        $race=Race::where('public_results_token',$token)->whereNotNull('results_published_at')->firstOrFail();
        $race->load('checkpoints');
        return Inertia::render('Results/Index',[
            'publicMode'=>true,
            'race'=>array_merge($race->only(['id','name','event_date','timezone','status','started_at','finished_at','settings']),['checkpoints'=>$race->checkpoints->map(fn($cp)=>$cp->only(['id','name','sequence','kind','discipline','distance_km']))]),
            'results'=>$results->filtered($race,$this->filters($request)),
            'filters'=>$this->filters($request),
            'categories'=>$race->entries()->whereNotNull('category')->distinct()->orderBy('category')->pluck('category'),
