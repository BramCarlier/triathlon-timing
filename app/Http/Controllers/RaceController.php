...
class RaceController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('Races/Index', ['races' => $query->get()]);
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $race->checkpoints()->createMany([
            ...
            ['name' => 'Bike Finish', 'code' => 'BIKE_FINISH', 'sequence' => 20, 'kind' => CheckpointKind::Transition, 'discipline' => Discipline::Bike, 'distance_km' => $data['bike_km'] ?? 35, 'is_required' => true],
            ['name' => 'Finish', 'code' => 'FINISH', 'sequence' => 30, 'kind' => CheckpointKind::Finish, 'discipline' => Discipline::Run, 'distance_km' => $data['run_km'] ?? 8, 'is_required' => true],
        ]);
    ...
    public function show(Request $request, Race $race): Response
    {
        ...
        $organizers = $request->user()->isAdmin() ? User::whereIn('role', ['admin', 'organizer'])->orderBy('name')->get(['id','name','email']) : [];
    ...
    public function update(Request $request, Race $race): RedirectResponse
    {
        ...
        $data = $request->validate(['name' => ['required', 'string', 'max:255'], 'event_date' => ['required', 'date'], 'timezone' => ['required', 'timezone'], 'status' => ['required', Rule::in($allowedStatuses)], 'organizer_ids' => ['nullable', 'array'], 'organizer_ids.*' => ['integer', 'exists:users,id']]);
