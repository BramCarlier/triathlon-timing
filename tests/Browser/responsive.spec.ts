...
test('every reachable page fits portrait and landscape with long names, dialogs and touch controls',async({page},info)=>{
 ...
 for(const size of sizes) {
  ...
  await page.getByRole('button',{name:'View splits',exact:true}).first().click();await expect(page.getByText('split 00:16:40.234',{exact:true}).first()).toBeVisible();await layout(page,`${size.name} expanded results`);
