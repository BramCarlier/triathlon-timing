...
async function accountMenu(page:Page) {
 ...
 if(await summary.getAttribute('aria-expanded')!=='true' && !(await summary.evaluate(el=>(el.parentElement as HTMLDetailsElement)?.open))) await summary.click();
}
test('every reachable page fits portrait and landscape with long names, dialogs and touch controls',async({page},info)=>{
 ...
 for(const size of sizes) {
  ...
  await menu(page);await expect(page.getByRole('link',{name:'People',exact:true})).toBeVisible();await layout(page,`${size.name} open menu`);
  await page.getByRole('link',{name:'People',exact:true}).click();await expect(page).toHaveURL(/\/users$/);await expect(page.getByRole('heading',{name:'People & access',exact:true})).toBeVisible();
  ...
  await page.goto('/races/9002');await page.locator('summary').filter({hasText:'More tools'}).click();await page.getByRole('button',{name:'Delete race',exact:true}).click();
  ...
  await page.goto('/races/9001/results');await page.getByText('Display options',{exact:true}).click();await page.getByLabel('Timing precision').selectOption('3');
  ...
  await page.goto('/users/9001/edit');await page.getByLabel('Role',{exact:true}).selectOption('athlete');await expect(page.getByLabel('Athlete',{exact:true})).toBeVisible();await layout(page,`${size.name} athlete picker`);
  await page.goto('/races/9002/participants/import');await page.getByLabel('Participant file').setInputFiles({name:'long-participant-import-preview.csv',mimeType:'text/csv',buffer:Buffer.from('type,first_name,last_name\nsolo,'+'LongName'.repeat(10)+',Runner\n')});await page.getByRole('button',{name:'Preview file',exact:true}).click();await expect(page.getByRole('button',{name:'Confirm import'})).toBeVisible();await layout(page,`${size.name} import preview`);
 }
 await page.setViewportSize(sizes[5]);await page.goto('/races');await accountMenu(page);await page.getByRole('button',{name:'Log out',exact:true}).click();await expect(page).toHaveURL(/\/login$/);
 await login(page,'responsive-official@example.test');
 for(const size of sizes){await page.setViewportSize(size);for(const path of ['/races','/guide','/races/9001','/races/9001/station','/races/9001/results']){await page.goto(path);await layout(page,`${size.name} official ${path}`);}await expect(page.getByRole('link',{name:'Corrections & station health',exact:true})).toHaveCount(0);await expect(page.getByRole('link',{name:'Export CSV',exact:true})).toHaveCount(0);}
 await page.setViewportSize(sizes[5]);await accountMenu(page);await page.getByRole('button',{name:'Log out',exact:true}).click();await expect(page).toHaveURL(/\/login$/);
